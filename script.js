...
function rosterCallback(iq) {
    ...
    $(iq).find('item').each(function () {
        var jid = $(this).attr('jid'); // Die Jabber ID des Kontakts
...
function onSubscriptionRequest(stanza) {
    if (stanza.getAttribute("type") == "subscribe") {
        ...
        log('subscriptionRequest: from=' + from);
        // Rücksenden einer 'subscribed' notification um die Kontaktanfrage anzunehmen.
        connection.send($pres({
            to: from,
            type: "subscribed"
        }));
...
//Rawinput und Output: Schreibt rohe Daten nur in die Datenkonsole (von Stophe benötigt)
